...
module.exports = function () {
    var path = require('path'), fs = require('fs'), clone = require('copy-anything').copy;
    var less = require('../');
    var stylize = require('../lib/less-node/lessc-helper').stylize;
    var globals = Object.keys(global);
    var oneTestOnly = process.argv[2], isFinished = false;
    ...
    var queueList = [], queueRunning = false;
    function queue(func) {
        if (queueRunning) {
    ...
    var totalTests = 0, failedTests = 0, passedTests = 0, finishTimer = setInterval(endTest, 500);
    ...
    function testSourcemap(name, err, compiledLess, doReplacements, sourcemap, baseFolder) {
        ...
        var expectedSourceMapURL = name + '.css.map', sourceMappingPrefix = '/*# sourceMappingURL=', sourceMappingSuffix = ' */', expectedCSSAppendage = sourceMappingPrefix + expectedSourceMapURL + sourceMappingSuffix;
        if (!compiledLess.endsWith(expectedCSSAppendage)) {
            ...
            var startOfSourceMappingValue = indexOfSourceMappingPrefix + sourceMappingPrefix.length, indexOfNextSpace = compiledLess.indexOf(' ', startOfSourceMappingValue), actualSourceMapURL = compiledLess.substring(startOfSourceMappingValue, indexOfNextSpace === -1 ? compiledLess.length : indexOfNextSpace);
    ...
    function testImports(name, err, compiledLess, doReplacements, sourcemap, baseFolder, imports) {
        ...
        var importsString = stringify(imports.sort());
    ...
    function testTypeErrors(name, err, compiledLess, doReplacements, sourcemap, baseFolder) {
        var fileSuffix = semver.gte(process.version, 'v16.9.0') ? '-2.txt' : '.txt';
    ...
    function globalReplacements(input, directory, filename) {
        ...
        var p = filename ? path.join(path.dirname(filename), '/') : directory, pathimport = path.join(directory + 'import/'), pathesc = p.replace(/[.:/\\]/g, function (a) { return '\\' + (a == '\\' ? '\/' : a); }), pathimportesc = pathimport.replace(/[.:/\\]/g, function (a) { return '\\' + (a == '\\' ? '\/' : a); });
        ...
        fs.readdirSync(path.join(baseFolder, foldername)).forEach(function (file) {
            ...
            queue(function () {
                toCSS(options, path.join(baseFolder, foldername + file), function (err, result) {
                     ...
                     */
                    if (verifyFunction) {
                        var verificationResult = verifyFunction(name, err, result && result.css, doReplacements, result && result.map, baseFolder, result && result.imports);
                        release();
    ...
    function toCSS(options, filePath, callback) {
        ...
        options.paths = options.paths.map(function (searchPath) {
    ...
    function testImportRedirect(nockScope) {
        return function (name, err, css, doReplacements, sourcemap, baseFolder) {
            process.stdout.write('- ' + path.join(baseFolder, name) + ': ');
            if (err) {
    ...
    function testDisablePluginRule() {
        less.render('@plugin "../../plugin/some_plugin";', { disablePluginRule: true }, function (err) {
            // TODO: Need a better way of identifing exactly which error is thrown.  Checking
            // text like this tends to be rather brittle.
            var EXPECTED = '@plugin statements are not allowed when disablePluginRule is set to true';
