            ...
            options: {
                urls: testName === "all"
                    ? browserTests.map(function (name) {
                        return ("http://localhost:8081/tmp/browser/test-runner-" +
                            name +
                            ".html");
                    })
                    : [
                ...
                build: process.env.TRAVIS_BRANCH === "master"
        ...
        shell: {
            ...
            opts: {
                ...
                command: [
                    ...
                    // CURRENT OPTIONS
                    "node bin/lessc --ie-compat ".concat(lessFolder, "/_main/lazy-eval.less tmp/lazy-eval.css"),
                    // --math
                    "node bin/lessc --math=always ".concat(lessFolder, "/_main/lazy-eval.less tmp/lazy-eval.css"),
                    "node bin/lessc --math=parens-division ".concat(lessFolder, "/_main/lazy-eval.less tmp/lazy-eval.css"),
                    "node bin/lessc --math=parens ".concat(lessFolder, "/_main/lazy-eval.less tmp/lazy-eval.css"),
                    "node bin/lessc --math=strict ".concat(lessFolder, "/_main/lazy-eval.less tmp/lazy-eval.css"),
                    "node bin/lessc --math=strict-legacy ".concat(lessFolder, "/_main/lazy-eval.less tmp/lazy-eval.css"),
                    // DEPRECATED OPTIONS
                    // --strict-math
                    "node bin/lessc --strict-math=on ".concat(lessFolder, "/_main/lazy-eval.less tmp/lazy-eval.css")
                ].join(" && ")
            },
            plugin: {
                command: [
                    "node bin/lessc --clean-css=\"--s1 --advanced\" ".concat(lessFolder, "/_main/lazy-eval.less tmp/lazy-eval.css"),
                    "cd lib",
                    "node ../bin/lessc --clean-css=\"--s1 --advanced\" ../".concat(lessFolder, "/_main/lazy-eval.less ../tmp/lazy-eval.css"),
                    "node ../bin/lessc --source-map=lazy-eval.css.map --autoprefix ../".concat(lessFolder, "/_main/lazy-eval.less ../tmp/lazy-eval.css"),
                    "cd ..",
                    // Test multiple plugins
                    "node bin/lessc --plugin=clean-css=\"--s1 --advanced\" --plugin=autoprefix=\"ie 11,Edge >= 13,Chrome >= 47,Firefox >= 45,iOS >= 9.2,Safari >= 9\" ".concat(lessFolder, "/_main/lazy-eval.less tmp/lazy-eval.css")
                ].join(" && ")
            },
            "sourcemap-test": {
                // quoted value doesn't seem to get picked up by time-grunt, or isn't output, at least; maybe just "sourcemap" is fine?
                command: [
                    "node bin/lessc --source-map=test/sourcemaps/maps/import-map.map ".concat(lessFolder, "/_main/import.less test/sourcemaps/import.css"),
                    "node bin/lessc --source-map ".concat(lessFolder, "/sourcemaps/basic.less test/sourcemaps/basic.css")
                ].join(" && ")
            }
        },
        eslint: {
